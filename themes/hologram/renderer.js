/**
 * Hologram Theme
 * Chromatic aberration, scanline sweep, translucent flicker, sci-fi blue
...
;(function(global) {
  ...
  var HologramTheme = {
    ...
    init(container, text, config) {
      ...
      container.classList.add('theme-hologram');

      var color = '#' + (config.color || this.defaults.color);
      var bg = '#' + (config.bg || this.defaults.bg);
      container.style.setProperty('--holo-color', color);
      container.style.setProperty('--holo-bg', bg);
      container.style.backgroundColor = bg;

      // Add scanline sweep overlay
      var scanline = document.createElement('div');
      scanline.className = 'holo-scanline';
      container.appendChild(scanline);
      this._scanlineEl = scanline;
    ...
    _initFlow(container, text, config) {
      ...
      track.className = 'holo-flow-track';

      for (var i = 0; i < 2; i++) {
        var span = document.createElement('span');
        span.className = 'holo-flow-text';
    ...
    togglePause() {
      ...
      // Also pause scanline
      if (this._scanlineEl) {
        this._scanlineEl.style.animationPlayState = this._paused ? 'paused' : 'running';
      }
      return this._paused;
    },
