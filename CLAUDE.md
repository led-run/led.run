...
- **Landing page URL builder** — inline theme/color/mode selector below input; updates URL preview in real-time; GO button navigates with full params; builder is collapsed by default (simple mode), expandable via "Advanced Options" toggle link; mode preference persisted in `localStorage('led-builder-mode')` as `'advanced'` or `'simple'`; simple mode GO navigates with text only (no query params); advanced mode GO includes all configured params
